...
// Vierde markt op dezelfde motor als Energie, Engineering en Architectuur.
// Onderwerp: bouwovertredingen regulariseren. Drie eigen sites spelen hier mee
// (de specialist, de zelfcheck en het moederbureau); regulariseren.be is de
// maat waartegen we de markt afzetten.
const MARKT = "regularisatie" as const;
const ONS_DOMEIN = "regulariseren.be";
const PAD = "/regularisatie/concurrentie";
...
const regPaginas = (n: number | null | undefined) =>
  n === 1 ? "regularisatiepagina" : "regularisatiepagina's";

export default async function RegularisatieConcurrentiePage({
  searchParams,
}: {
    ...
    return (
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900">
          ...
          <p className="mt-1">
            Voor regularisatie bestaat geen register. De lijst begint bij het concurrentieonderzoek
            van augustus 2026 en groeit daarna met wie er op onze zoektermen in Google staat en
            wat de crawler op de sites zelf vindt. Draai{" "}
            <code className="rounded bg-white px-1.5 py-0.5">/api/concurrentie?import=1&amp;crawl=0</code>{" "}
            om de startlijst te laden, en{" "}
            <code className="rounded bg-white px-1.5 py-0.5">/api/zoekwoorden?markt=regularisatie&amp;posities=1</code>{" "}
  ...
  const eigen = getConcurrentenInMarkt(MARKT, "eigen");
  const wij = eigen.find((c) => c.domein === ONS_DOMEIN);
  ...
  const bronnen = [
    ...
    },
    {
      ...
      uitleg: zwStatus.met_volume > 0
        ...
        : "draai /api/zoekwoorden?markt=regularisatie&volumes=1",
    },
    {
      naam: "Onze posities (Search Console)",
      klaar: gsc.klaar && gscCijfers.metingen > 0,
      uitleg: gscCijfers.metingen > 0
        ? `${num(gscCijfers.metingen)} termen op ${num(gscCijfers.sites)} eigen ${gscCijfers.sites === 1 ? "site" : "sites"}`
  ...
  const uitOnderzoek = bronnen0.find((b) => b.bron === "onderzoek")?.n || 0;
    ...
    <main className="mx-auto max-w-7xl px-6 py-8">
      <div className="kopbalk">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <div className="text-xs text-zinc-500">
            Bouwovertredingen regulariseren · laatste sitecontrole{" "}
      ...
      <section id="leaders" className="scroll-mt-36 pt-8">
        ...
        <div className="mb-4 rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
          ...
          <p className="mt-1">
            Voor regularisatie bestaat geen register: iedere architect mag een bouwovertreding
            ...
            crawler zelf genoeg regularisatiewerk vond. Wie niet online staat en niet rankt, staat hier
            niet — maar die neemt ook geen zoekende eigenaar weg.
          </p>
        </div>
        ...
        <Card title="Wie leidt er online">
          {leaderboard.length > 0 ? (
            <>
              <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2 text-xs">
                ...
                <a
                  href={toonAlles ? PAD : `${PAD}?toon=alles`}
              ...
              <p className="mt-3 text-[11px] text-zinc-500">
                &ldquo;Wij&rdquo; is hier de hele groep: regulariseren.be, mijnregularisatie.be én
                h-architects.be. Staan twee van die drie in dezelfde top 5, dan is dat geen dubbele
                winst maar een teken dat de sites elkaar beconcurreren.
              </p>
            </>
          ) : (
            <>
              <div className="mb-3 text-xs text-zinc-500">
                ...
                zelf meten: hoeveel pagina&rsquo;s een site over regularisatie heeft, hoeveel ze
          ...
          {onzePosities.length > 0 && (
            <div className="mt-4 rounded-lg border border-zinc-200 p-3">
                ...
                <tbody>
                  {onzePosities.map((r) => (
                    <tr key={r.site + r.term} className="border-b border-zinc-100 last:border-0">
                      ...
                      <td className="py-1 pr-4 whitespace-nowrap text-[11px] text-zinc-400">
                        {r.site.replace(/^sc-domain:/, "").replace(/^https?:\/\//, "").replace(/\/$/, "")}
                      </td>
              ...
              <p className="mt-2 text-[11px] text-zinc-500">
                Alleen de regularisatietermen van onze drie sites. De ontwerptermen van h-architects.be
                staan op de Architectuur-pagina.
              </p>
            </div>
          )}
          ...
          <div className="mt-3 flex items-baseline gap-2 rounded-lg bg-zinc-50 p-3 text-xs text-zinc-600">
            ...
            <span>
              {wij
                ? `${num(wij.omvang || 0)} ${regPaginas(wij.omvang)}, ` +
      ...
      <section id="markt" className="scroll-mt-36 pt-8">
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
          ...
          />
          <Kpi
            label="Gem. regularisatiepagina's"
          ...
          <Kpi
            ...
            sub={`regularisatiepagina's op ${ONS_DOMEIN}`}
          />
        </div>

        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <Card title="Grootst online in regularisatie">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-200 text-left text-xs uppercase tracking-wide text-zinc-400">
                    <th className="pb-2 pr-3 font-medium">Site</th>
                    <th className="pb-2 pr-3 text-right font-medium">Reg.-pag.</th>
            ...
            <p className="mt-3 border-t border-zinc-100 pt-3 text-xs text-zinc-500">
              Gerangschikt op pagina&rsquo;s over regularisatie, niet op het totale aantal
              pagina&rsquo;s. Een architectenbureau met één dienstenpagina haalt hier één; een
              specialist die per situatie en per doelgroep schrijft haalt er tien. Dat verschil is de
              markt.
            </p>
          </Card>
          ...
          <Card title="Wie er nog beweegt">
            {actiefste.length === 0 ? (
              <p className="text-sm text-zinc-500">
                Geen enkele site in deze lijst publiceerde het afgelopen jaar. Dat is op zich het
              ...
              <>
                ...
                <p className="mt-3 border-t border-zinc-100 pt-3 text-xs text-zinc-500">
                  Alleen sites die het afgelopen jaar nog gepubliceerd hebben. Omvang zegt hoe groot
                  iemand is; dit zegt of hij nog beweegt. Let op de advocaten en de
                  vastgoeddata-sites: die schrijven het meest over boete, dwangsom en het
                  maatregelenregister, en geen enkel architectenbureau doet dat.
                </p>
              </>
            )}
          </Card>
        ...
        <div className="mt-4">
          <Card title="Onze eigen sites">
            {eigen.length === 0 ? (
              <p className="text-sm text-zinc-500">Nog geen eigen site gemeten.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-200 text-left text-xs uppercase tracking-wide text-zinc-400">
                      ...
                      <th className="pb-2 pr-3 text-right font-medium">Pagina&rsquo;s</th>
                      <th className="pb-2 pr-3 text-right font-medium">Reg.-pag.</th>
                      <th className="pb-2 pr-3 text-right font-medium">Artikels</th>
                      <th className="pb-2 pr-3 font-medium">Laatste post</th>
                      <th className="pb-2 pr-3 text-right font-medium">Laadtijd</th>
                      <th className="pb-2 font-medium">CMS</th>
                    </tr>
                  </thead>
                  <tbody>
                    {eigen.map((s) => (
                      <tr key={s.domein} className="border-b border-zinc-100 last:border-0">
                        <td className="py-1.5 pr-3">
                          <a href={`https://${s.domein}`} target="_blank" rel="noreferrer noopener"
                             className={"hover:text-blue-700 hover:underline " + (s.domein === ONS_DOMEIN ? "font-semibold text-zinc-900" : "text-zinc-800")}>
                            {s.domein}
                          </a>
                        ...
                        <td className="py-1.5 pr-3 text-right tabular-nums font-semibold text-zinc-800">{num(s.omvang || 0)}</td>
                        <td className="py-1.5 pr-3 text-right tabular-nums text-zinc-600">{num(s.blog_artikels || 0)}</td>
                        <td className="py-1.5 pr-3 text-zinc-600"><Datum d={s.laatste_blog} href={s.laatste_blog_url} /></td>
                        <td className="py-1.5 pr-3 text-right tabular-nums text-zinc-600">{s.ttfb_ms ? `${num(s.ttfb_ms)} ms` : "—"}</td>
                        <td className="py-1.5 text-xs text-zinc-500">{s.cms || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <section id="concurrenten" className="scroll-mt-36 pt-8">
        <Card title={`Concurrenten — ${num(concurrenten.length)} sites in de regularisatiemarkt`}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-200 text-left text-xs uppercase tracking-wide text-zinc-400">
                  <th className="pb-2 pr-4 last:pr-0 whitespace-nowrap font-medium">Bedrijf</th>
                  <th className="pb-2 pr-4 last:pr-0 whitespace-nowrap text-right font-medium">Reg.-pag.</th>
      ...
      <section id="nakijken" className="scroll-mt-36 pt-8">
        <Card title={`Nakijken — ${num(rest.length)} sites die de automatiek buiten de markt hield`}>
          <p className="mb-4 text-sm text-zinc-600">
            ...
            <strong className="font-medium text-zinc-800"> Portalen en vastgoeddata-sites horen hier
            thuis</strong>, maar lees ze wel: Spotto en de notariële blogs schrijven als enigen over
            het maatregelenregister, en dat is precies de opening waar regulariseren.be op inzet.
            Zit er een echte concurrent tussen, zet hem dan hier recht; hij verschuift meteen naar de
                    ...
                    <td className="py-2 pr-4 last:pr-0">
                      <span className="rounded bg-zinc-100 px-1.5 py-0.5 text-[11px] text-zinc-600">
                        {c.categorie === "geen-concurrent" ? "geen concurrent (handmatig)"
                          : c.categorie === "aannemer" ? "aannemer — bouwt, regulariseert niet"
      ...
      <section id="diensten" className="scroll-mt-36 pt-8">
        <Card title="Dienstendekking — wat biedt de markt aan, en waar zit het gat">
          ...
          <p className="mt-4 border-t border-zinc-100 pt-3 text-xs text-zinc-500">
            Herkend op basis van de homepage en de URL-structuur van de sites in deze markt. Kijk
          ...
          <div className="mt-4">
            <Card title="Herschrijfkansen — overheid en portalen die onze zoektermen bezetten">
              <p className="mb-3 text-sm text-zinc-600">
                ...
                zoektermen horen. Ambtelijk of juridisch geschreven, zelden vanuit de eigenaar die
                met een bouwovertreding zit. Dat is precies waar wij een duidelijker antwoord
      ...
      <section id="zoekwoorden" className="scroll-mt-36 pt-8">
        <Card title={`Zoekwoorden — ${num(zwStatus.termen)} termen die deze markt afbakenen`}>
          ...
          {bron.klaar && !zwStatus.positie_datum && (
            <div className="mb-4 rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
              ...
              <p className="mt-1">
                ...
                Regularisatie wordt daarom om de twee weken gemeten, op de termen met het meeste
                volume. Handmatig starten kan met{" "}
                <code className="rounded bg-white px-1.5 py-0.5">/api/zoekwoorden?markt=regularisatie&amp;posities=1&amp;limiet=15</code>.
              </p>
            </div>
          )}
          ...
          {zwStatus.met_volume === 0 && (
            <div className="mb-4 rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
              ...
              <p className="mt-1">
                ...
                <code className="rounded bg-white px-1.5 py-0.5">/api/zoekwoorden?markt=regularisatie&amp;volumes=1</code>{" "}
          ...
          <p className="mt-3 text-xs text-zinc-500">
            De lijst staat in <code>config/zoekwoorden-regularisatie.json</code> — termen bijzetten
            kan zonder code. De termen met de intentie{" "}
            <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[11px] text-amber-800">probleem</span>{" "}
            zijn hier de waardevolste: wie &ldquo;huis kopen met bouwovertreding&rdquo; of
            &ldquo;dwangsom bouwovertreding&rdquo; intikt, heeft het probleem al en zoekt iemand die
            het oplost. &ldquo;Wij&rdquo; is de beste positie van onze drie sites samen.
          </p>
        </Card>
        ...
        <Card title="Signalen — wat er veranderd is sinds de vorige controle">
          {signalen.length === 0 ? (
            <p className="text-sm text-zinc-500">
              ...
              nieuwe pagina&rsquo;s — bijvoorbeeld de dag dat een concurrent alsnog een pagina over
              het maatregelenregister zet.
            </p>
          ) : (
